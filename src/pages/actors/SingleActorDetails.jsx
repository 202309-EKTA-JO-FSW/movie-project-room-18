...
async function SingleActorDetails(id) {
...
async function SingleActorDetailsImage(id) {
  const API_KEY = "94efbb958c3750928aa5e84dbe4193bf";
  const BASE_URL = "https://api.themoviedb.org/3";
...
async function displayActorDetails(actorId) {
  try {
    const actorInfo = await SingleActorDetails(actorId);
    const actorImage = await SingleActorDetailsImage(actorId);
